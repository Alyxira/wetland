<template>
  <div class="chart-container">
    <div class="card-header">
      <h3 class="card-title">湿地植被地上碳储量变化趋势</h3>
      <span class="card-subtitle">VEGETATION CARBON STORAGE</span>
    </div>
...
const fallbackSection = getFallbackScreenDataset().vegetationCarbon
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `<div style="font-size: 16px; font-weight: 600;">${item.axisValue} 年</div>
                <div style="font-size: 14px; margin-top: 4px;">碳储量：${item.value.toLocaleString()} 万吨</div>`
      }
    },
    grid: {
      left: '16%',
...
onMounted(() => {
  loadScreenDataset().then((dataset) => {
    initChart(dataset.vegetationCarbon)
