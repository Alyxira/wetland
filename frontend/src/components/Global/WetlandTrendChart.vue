<template>
  <div class="chart-container">
    <div class="card-header">
      <h3 class="card-title">湿地增长趋势</h3>
      <span class="card-subtitle">GROWTH TREND</span>
    </div>
    <div ref="chartRef" class="chart-body"></div>
    <div class="card-footer">
      <a href="https://www.gov.cn/xinwen/2023-02/02/content_5739608.htm" target="_blank" class="source-link">
        数据来源：国家林草局《国际重要湿地名录》
      </a>
    </div>
  </div>
...
const fallbackSection = getFallbackScreenDataset().wetlandTrend
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `<div style="font-size: 16px; font-weight: 600;">${item.axisValue} 年</div>
                <div style="font-size: 14px; margin-top: 4px;">国际重要湿地：${item.value} 处</div>`
      }
    },
    grid: {
      left: '5%',
...
onMounted(async () => {
  ...
  initChart(dataset.wetlandTrend)
